...
public abstract class StringUtils {
    ...
    public static boolean isIpInRange(String ipAddr, String cidrAddr) {
        ...
        if(!matcher.matches()) {
            ...
            int i;
            for(int realIpParts = 0; realIpParts < ipParts.length; ++realIpParts) {
                i = Integer.parseInt(ipParts[realIpParts]);
                if(intMask >= 8) {
                    minIpParts[realIpParts] = i;
                    maxIpParts[realIpParts] = i;
                    intMask -= 8;
                } else if(intMask > 0) {
                    minIpParts[realIpParts] = i >> intMask;
                    maxIpParts[realIpParts] = i | 255 >> intMask;
                    intMask = 0;
                } else {
                    minIpParts[realIpParts] = 1;
                    maxIpParts[realIpParts] = 254;
                }
            }

            String[] var10 = ipAddr.split("\\.");

            for(i = 0; i < var10.length; ++i) {
                int realIp = Integer.parseInt(var10[i]);
